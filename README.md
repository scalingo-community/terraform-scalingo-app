...
The module computes an estimation of the monthly cost of the resources it manages, exposed through the `estimated_monthly_cost` output:
...
The output contains a `total` (and a `total_max` when autoscalers are configured, computed with `max_containers`), a per-container and per-addon breakdown, and the list of `unpriced` sizes/plans for which no rate is known (they are excluded from the totals).

The public rates of the osc-fr1 region are shipped with the module in `pricing_defaults.tf`. This file is generated by `scripts/update-pricing.sh` from the Scalingo API and refreshed monthly by the *Update pricing* GitHub Actions workflow. You can override any rate (negotiated prices, other region...) through the `pricing` variable:
...
module "my_app" {
  ...
  pricing = {
    # override the monthly price of a container size
    container_sizes = {
      L = 25.00
    }
    # override the monthly price of an addon plan
    addon_plans = {
      postgresql-starter-512 = 6.50
    }
  }
}
```
