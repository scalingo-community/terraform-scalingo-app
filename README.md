...
| <a name="input_region"></a> [region](#input\_region) | Scalingo region where the application is deployed (default: "osc-fr1"). Must match the region configured on the Scalingo provider: the `region` and `origin_domain` outputs are derived from it. Set it explicitly if you deploy to another region such as "osc-secnum-fr1". | `string` | `"osc-fr1"` | no |
